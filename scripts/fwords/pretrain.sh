...
EXP_NAME=mnli-glove        # experiment name
RUN_NAME=mnli-glove-train  # run name

OVERRIDES+=", exp_name = ${EXP_NAME}"
OVERRIDES+=", run_name = ${RUN_NAME}"
OVERRIDES+=", pretrain_tasks=${PRETRAIN_TASK}"
OVERRIDES+=', target_tasks=none'
OVERRIDES+=", sent_enc=glt, elmo_chars_only=1, dropout=0.2, random_seed=1234, cuda=${CUDA_NO}, reload_vocab=0, reload_tasks=0, training_data_fraction=1, do_target_task_training=0, do_full_eval=0, load_model=1"
